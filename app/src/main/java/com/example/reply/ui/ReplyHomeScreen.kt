...
fun ReplyHomeScreen(
    ...
    if (navigationType == ReplyNavigationType.PERMANENT_NAVIGATION_DRAWER
        ...
        // dung 1 thuoc tinh cua UiState de compose check va hien thi
        // theo: dang la Man hinh chinh hay ko.
        // screens are recomposed when a state change occurs
        // You'll use conditionals to show the content of the home screen
        // when the user is at the home screen, and the details screen
        // when the user is not at the home screen.
        if (replyUiState.isShowingHomepage) {
            ReplyAppContent(
                navigationType = navigationType,
                replyUiState = replyUiState,
                onTabPressed = onTabPressed,
                onEmailCardPressed = onEmailCardPressed,// bam vao 1 email se chay ham
                // viewModel.updateDetailsScreenStates(email)
                // _uiState.update { isShowingHomepage = false}
                // nen se chuyen sang trang chi tiet ReplyDetailsScreen
                navigationItemContentList = navigationItemContentList,
                modifier = modifier
            )
        } else {
